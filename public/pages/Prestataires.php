    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        ...
        foreach ($prestataires as $row): 
            if ($currentUser !== $row['user_id']): 
                if ($currentUser !== null) echo '</ul><a href="' . BASE_URL . '/contact?prestataire=' . urlencode($currentUser) . '" class="btn-contact">Contacter</a></div>'; 
        ?>
            <!-- Carte Prestataire -->
        ...
        <?php endforeach; ?>
                ...
                <a href="<?= BASE_URL ?>/contact?prestataire=<?= $currentUser ?>" 
